...
$reportfile = "$env:USERPROFILE\Desktop\License.xlsx"
 
ForEach ($client in $clients)
{
	$ClientName = $client.Name
	Write-Host "Working on $ClientName" -ForegroundColor Yellow
	$Users = Get-MsolUser -TenantId $client.TenantId -All| Where-Object { $_.isLicensed -eq "TRUE" } | Sort-Object DisplayName
	Foreach ($User in $Users)
	{
			...
			Foreach ($License in $Licenses)
			{
				...
				If (!($TextLic))
				{
                   ...
                   # $NewObject02 | Export-CSV $CSVName -NoTypeInformation -Append
                    $NewObject02 | Export-Excel -Path $reportfile -WorksheetName $ClientName -TableName $ClientName -Append
				}
				Else
				{
					$CSVName = "$CSV" + "$ClientName.csv"
		...
		Else
		{
			...
			If (!($TextLic))
			{
				...
				$NewObject02 += $NewObject01
                #$NewObject02 | Export-CSV $CSVName -NoTypeInformation -Append
                $NewObject02 | Export-Excel -Path $reportfile -WorksheetName $ClientName -TableName $ClientName -a
			}
			Else
			{
				$CSVName = "$CSV" + "$ClientName.csv"
