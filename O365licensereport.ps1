# code to run the function is last in this file
...
function Get-UserLicenseReport {
    ...
    #Create Hash table with friendly names
    $Sku = @{
    ...
    #Get month and year in a variable
    $Columndate = Get-Date -Format "MM.yyyy"

    #Create an array (a list) to add the license rows to
    $licensereport = @()
    ...
    # for every user found, run the loop (foreach) below
    ForEach ($User in $userList) {
        #get the license for each user
        $lic = Get-AzureADUser -ObjectId $User.UserPrincipalName | Select-Object -ExpandProperty AssignedLicenses | Select-Object SkuID 
        
        #for every licence plan the current user in the loop have been assigned, run the loop below
        foreach ($plan in $lic) {
            #for every license found, collect the user data and license in a PSObject
            ...
            #Get the friendly name for this particular license from the list above
            $TextLic = $Sku.Item($License.SkuPartNumber)
            if (!$TextLic) {
                # There is no friendly name for this license in the list so just use the actual name
            ...
            #Report row created, lets add it to the report list
            $LicenseReport += $NewObject01
        #End of license plan loop
        } 
    #End of user loop
    }
        
    #Export the license report list to excel
    $LicenseReport
